...
static const yytype_uint16 yyrline[] =
{
     ...
     183,   190,   200,   205,   211,   221,   227,   233,   239,   251,
     262,   274,   284,   294,   304,   310,   319,   323,   334,   340,
     350,   356,   366,   370,   380,   386,   396,   400,   410,   414,
     421,   428,   432,   441,   449,   453,   457,   464,   473,   478,
     482,   490
};
#endif
...
#line 191 "syntax_analyzer.y"
    {
			...
			node->addChild((yyvsp[(1) - (4)])->getNode());
			node->addChild(TreeNode::createTerminalNode("LTHIRD", "["));
			node->addChild(TreeNode::createTerminalNode("CONST_INT", (yyvsp[(3) - (4)])->getname().c_str())); //PROBLEM
			node->addChild(TreeNode::createTerminalNode("RTHIRD", "]"));
			(yyval) = new symbol_info("declaration_list", "non_terminal", node); 
		}
    break;
...
#line 240 "syntax_analyzer.y"
    {
	        TreeNode* node = TreeNode::createNonTerminalNode("statement"); 
			node->addChild(TreeNode::createTerminalNode("FOR", "for")); //PROBLEM
			node->addChild(TreeNode::createTerminalNode("LPAREN", "("));
...
int main(int argc, char *argv[])
{
    ...
    yyin = fopen(argv[1], "r");
    outlog.open("21301191_log.txt", std::ios::trunc);
